...
function getExperienceCopy(locale) {
  const projectLabels = {
    agentToolchain: t(locale, 'pdf.product.agentToolchain.label'),
    symbiote: t(locale, 'pdf.product.symbiote.label'),
    videoStudio: t(locale, 'pdf.product.videoStudio.label'),
    messaging: t(locale, 'pdf.product.messaging.label'),
  };
  return [
    {
      ...
      projectsLabel: t(locale, 'pdf.role.rndPro.projectsLabel'),
      projects: PORTFOLIO_RND_PRO_PRODUCT_KEYS.map((key) => ({
        label: projectLabels[key],
        href: makePortfolioUrl(locale, PORTFOLIO_PROFILE_ITEM_ROUTES.products[key]),
      })),
    },
    {
...
function formatItemProjects(item) {
  if (!item.projects?.length) return '';
  const projects = item.projects
    .map(({ label, href }) => `[${label}](${href})`)
    .join(' · ');
  return `${item.projectsLabel}: ${projects}`;
}
...
function createWriter(doc) {
  ...
  function measureItemBrief(item, options = {}) {
    ...
    const projects = formatItemProjects(item);
    const projectsHeight = projects
      ? textHeight(projects, {
        size: options.projectsSize || 9,
        lineGap: options.projectsLineGap ?? 0.8,
      }) + (options.projectsTopGap ?? 2)
      : 0;
    return titleHeight + 3 + detailsHeight + projectsHeight + (options.after ?? 7);
  }
  ...
  function itemBrief(item, options = {}) {
    ...
    paragraph(item.details, {
      size: options.size || 9.8,
      color: COLORS.muted,
      after: item.projects?.length ? (options.projectsTopGap ?? 2) : (options.after ?? 7),
      lineGap: options.lineGap ?? 1.1,
    });
    if (item.projects?.length) {
      paragraph(formatItemProjects(item), {
        size: options.projectsSize || 9,
        color: COLORS.muted,
