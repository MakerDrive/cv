...
function projectDetails(project, locale) {
  return stripPortfolioArticleBlockMarkers(
    loadProjectContent(project, locale)
  );
}

function publicationBody(publication, locale) {
  const localized = publication.locales?.[locale] || publication.locales?.en || {};
...
function getRNDProProjectLinks(locale) {
  return PORTFOLIO_RND_PRO_PRODUCT_KEYS.map((key) => ({
    label: t(locale, `profile.product.${key}.label`),
    href: urlFor(PORTFOLIO_PROFILE_ITEM_ROUTES.products[key], locale),
  }));
}

function appendCompactExperience(lines, locale) {
  for (const key of profileRoleKeys) {
    lines.push(`- **${t(locale, `profile.role.${key}.label`)}**: ${oneLine(t(locale, `profile.role.${key}.details`))}`);
    if (key !== 'rndPro') continue;
    const projects = getRNDProProjectLinks(locale)
      .map(({ label, href }) => `[${label}](${href})`)
      .join(' · ');
    lines.push(`  - **${t(locale, 'profile.role.rndPro.projectsLabel')}**: ${projects}`);
  }
}

function appendFullExperience(lines, locale) {
  for (const key of profileRoleKeys) {
    lines.push(`#### ${t(locale, `profile.role.${key}.label`)}`, '');
    lines.push(t(locale, `profile.role.${key}.details`), '');
    if (key !== 'rndPro') continue;
    lines.push(`##### ${t(locale, 'profile.role.rndPro.projectsLabel')}`, '');
    for (const project of getRNDProProjectLinks(locale)) {
      appendListItem(lines, project.label, project.href);
    }
    lines.push('');
  }
}
...
export function writeLlmsTxt(projects, publications = PUBLICATIONS) {
  ...
  appendCompactExperience(lines, 'en');
  ...
  appendListItem(lines, t('en', 'skill.digitalPresence.label'), urlFor('skills/digital-presence', 'en'), t('en', 'skill.digitalPresence.summary'));
...
function writeFullLocale(lines, projects, publications, locale) {
  ...
  appendFullExperience(lines, locale);
  ...
  for (const pub of publicPublications) {
    ...
    lines.push(
      shiftHeadings(publicationBody(pub, locale)),
      '',
    );
