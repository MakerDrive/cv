...
test('portfolio build generates localized CV PDFs', async () => {
  ...
  assert.ok(pdfSource.indexOf("t(locale, 'profile.statusTitle')") < pdfSource.indexOf("t(locale, 'profile.focusTitle')"));
  assert.match(pdfSource, /t\(locale, 'profile\.focusDetails'\)/);
  assert.match(pdfSource, /t\(locale, 'profile\.workFormatTitle'\)/);
  assert.match(pdfSource, /t\(locale, 'profile\.achievementsTitle'\)/);
