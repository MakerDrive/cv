...
test('portfolio build generates the compact two-page localized CV structure', async () => {
  ...
  assert.match(pdfSource, /PORTFOLIO_RND_PRO_PRODUCT_KEYS/);
  assert.match(pdfSource, /projects: PORTFOLIO_RND_PRO_PRODUCT_KEYS\.map/);
  assert.match(pdfSource, /formatItemProjects\(item\)/);
