...
test('portfolio profile states full-stack work, native stack, and stack adaptation', () => {
  ...
  assert.match(PORTFOLIO_LOCALE_MESSAGES.en['portfolio.profile.ageLabel'], /Age/);
  ...
  assert.match(PORTFOLIO_LOCALE_MESSAGES.ru['portfolio.profile.ageLabel'], /Возраст/);
  ...
  assert.match(PORTFOLIO_LOCALE_MESSAGES.es['portfolio.profile.ageLabel'], /Edad/);
...
test('portfolio profile renders age and separated experience sections', async () => {
  ...
  assert.match(source, /const PORTFOLIO_ONLINE_CV_URL = 'https:\/\/MakerDrive\.github\.io\/cv\/';/);
  ...
  assert.match(source, /tPortfolio\('profile\.ageLabel'\)/);
