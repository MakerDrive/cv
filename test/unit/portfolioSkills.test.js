...
test('portfolio skills keep R&D central while separating hardware process automation', async () => {
  ...
  assert.match(source, /id: 'skills\/digital-presence'/);
...
test('portfolio skill routes match the active skill identifiers', async () => {
  ...
  await access(new URL('digital-presence/index.html.js', root));
...
test('portfolio exposes the complete profile without a redundant summary section', async () => {
  const source = await readFile(new URL('../../src/static-pages/js/index.js', import.meta.url), 'utf8');
  assert.doesNotMatch(source, /['"]bio\/about['"]/);
  await assert.rejects(
    access(new URL('../../src/static-pages/bio/about/index.html.js', import.meta.url)),
    { code: 'ENOENT' },
  );

  for (const locale of LOCALES) {
    const messages = PORTFOLIO_LOCALE_MESSAGES[locale];
    assert.equal(Object.hasOwn(messages, 'portfolio.bio.about'), false);
    assert.equal(Object.hasOwn(messages, 'portfolio.bio.summary'), false);
    assert.equal(Object.hasOwn(messages, 'portfolio.bio.details'), false);
  }
});
...
test('localized profile and PDF preserve the compact semantic structure', () => {
  for (const locale of LOCALES) {
    ...
    const collaborationTitle = {
      en: /engagement(?: options)?/i,
      ru: /формат сотрудничества/iu,
      es: /modalidad(?:es)? de colaboración/iu,
    };
    const collaborationMeaning = {
      ...
      es: /trabajo en remoto[\s\S]*hands-on lead[\s\S]*senior individual contributor[\s\S]*RND-PRO[\s\S]*proyecto(?:s)? (?:del cliente|de clientes)[\s\S]*mayor alcance/iu,
    };
    const fullStackDirection = {
      en: 'SMM and digital presence automation.',
      ru: 'SMM и автоматизация цифрового присутствия.',
      es: 'SMM y automatización de la presencia digital.',
    };
    ...
    assert.equal(keys.filter((key) => /^portfolio\.profile\.role\..+\.label$/.test(key)).length, 3);
    assert.equal(keys.filter((key) => /^portfolio\.pdf\.product\..+\.label$/.test(key)).length, 8);
    assert.equal(keys.filter((key) => /^portfolio\.pdf\.role\..+\.label$/.test(key)).length, 3);
    ...
    assert.match(messages['portfolio.profile.role.rndPro.label'], /2023/);
    assert.match(messages['portfolio.profile.role.rndPro.details'], /Full-Stack Product Engineer.*2020.*2022/i);
    assert.match(messages['portfolio.profile.role.f360.label'], /F360/);
    assert.match(messages['portfolio.profile.role.f360.label'], /2021.*2022/);
    assert.match(messages['portfolio.profile.role.megavisor.label'], /MEGAVISOR \/ Gate9.*(?:Technical Director|технический директор|director técnico)/i);
    assert.match(messages['portfolio.profile.role.megavisor.label'], /2011.*2014/);
    assert.doesNotMatch(messages['portfolio.profile.role.megavisor.label'], /\bCTO\b/i);
    assert.doesNotMatch(profile, /ZIQ Design/i);
    assert.doesNotMatch(pdf, /ZIQ Design/i);
...
test('portfolio profile renders the agreed recruiter-facing hierarchy and chronology', async () => {
  ...
  const roles = ['rndPro', 'f360', 'megavisor']
