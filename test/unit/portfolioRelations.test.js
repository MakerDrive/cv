...
test('curated portfolio relations resolve to known project entries', () => {
  ...
  assert.deepEqual(PORTFOLIO_SKILL_PROJECT_RELATIONS['skills/digital-presence'], [
    'agent-portal',
    'symbiote-workspace',
...
test('profile and PDF route mappings resolve to canonical localized pages', () => {
  ...
  assert.deepEqual(
    PORTFOLIO_RND_PRO_PRODUCT_KEYS.map((key) => PORTFOLIO_PROFILE_ITEM_ROUTES.products[key]),
    [
...
test('portfolio runtime exposes curated backlinks without replacing skill inference', async () => {
  ...
  assert.match(source, /projects: getRNDProProjects\(\)/);
  assert.match(source, /if \(item\.projects\?\.length\)/);
});
