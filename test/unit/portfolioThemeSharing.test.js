...
import { configureBrowserLocalization } from 'symbiote-ui/ui/locale.js';
import {
  decodeCascadeThemeShare,
  encodeCascadeThemeShare,
} from 'symbiote-ui/themes/cascade-theme-share.js';
...
test('TreeView selection carries its source row separately from the article target', () => {
  let committed = null;
  const controller = createPortfolioNavigationController({
    hasEntry: (id) => id === 'skills/agentic-ai',
    createMediaFragment: () => '',
    getSelection: () => ({ selectedId: 'profile/photo', mediaFragment: '' }),
    prepareNavigation: () => 'https://example.com/skills/agentic-ai/',
    pushUrl() {},
  ...
  assert.equal(controller.selectTreeItem({
    id: 'skills/agentic-ai',
    options: {
      focus: true,
      treeSelectionId: 'Projects/AI Tools',
    },
  }), true);
  assert.equal(committed.id, 'skills/agentic-ai');
  assert.equal(committed.treeSelectionId, 'Projects/AI Tools');

  assert.equal(controller.select('skills/agentic-ai', { focus: true }), true);
  assert.equal(committed.id, 'skills/agentic-ai');
  assert.equal(committed.treeSelectionId, '');
});
...
test('index keeps direct public share labels without private share controls', async () => {
  ...
  assert.match(source, /editor\.setAttribute\('share-label'/);
  assert.match(
    pageSource,
    /<cascade-theme-widget\s+class="pulse-theme-widget"\s+share-label="Share theme"\s*>/,
  );
});
...
test('runtime rollback restores the rendered article and exact media focus state', () => {
  ...
  const runtime = {
    selectedId: 'projects/current',
    treeSelectionId: 'Projects/Current',
    mediaFragment: 'media-current',
