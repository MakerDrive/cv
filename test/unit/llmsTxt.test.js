...
test('portfolio build generates llms.txt and llms-full.txt', async () => {
  ...
  assert.match(llmsTxt, /pulse\/photopizza-retrospective\/\?lang=en/);
  assert.match(llmsTxt, /downloads\/vladimir-matiasevich-cv-en\.pdf/);
  assert.match(llmsTxt, /## Professional profile/);
  assert.match(llmsTxt, /## Expertise/);
  assert.match(llmsTxt, /## Outcomes/);
  assert.match(llmsTxt, /## Products and systems/);
  assert.match(llmsTxt, /## Professional experience/);
  assert.match(llmsTxt, /\*\*Projects\*\*: \[Agent Portal and MCP\/WebMCP automation toolchain\]\([^\n]+projects\/agent-portal\/\?lang=en\)/);
  assert.match(llmsTxt, /\[Symbiote Workspace \/ UI \/ Engine \/ Node\]\([^\n]+projects\/symbiote-workspace\/\?lang=en\)/);
  assert.match(llmsTxt, /\[Symbiote Video Studio\]\([^\n]+projects\/symbiote-video-studio\/\?lang=en\)/);
  assert.match(llmsTxt, /\[Lifecycle Messaging Platform\]\([^\n]+projects\/lifecycle-messaging-platform\/\?lang=en\)/);
  assert.doesNotMatch(llmsTxt, /ZIQ Design/);
  assert.match(llmsTxt, /Languages: Russian \(native\) · English \(written and async\) · Spanish \(A1, actively learning\)/);
  assert.match(llmsTxt, /Multilingual voice meetings: simultaneous AI interpretation/);
  assert.match(llmsTxt, /15\+ years in software development and engineering R&D/);
  ...
  assert.match(llmsTxt, /Lifecycle Messaging Platform/);
  assert.match(llmsTxt, /skills\/digital-presence\/\?lang=en/);
  assert.match(llmsTxt, /SMM & Digital Presence Automation/);
  ...
  assert.match(llmsFullTxt, /Многоязычные голосовые встречи: синхронный ИИ-перевод/);
  assert.match(llmsFullTxt, /15\+ лет в разработке ПО и инженерном R&D/);
  ...
  assert.match(llmsFullTxt, /#### RND-PRO — Lead R&D Engineer · январь 2023 — настоящее время/);
  assert.match(llmsFullTxt, /Full-Stack Product Engineer, январь 2020 — декабрь 2022/);
  assert.match(llmsFullTxt, /##### Проекты\n\n- \[Agent Portal и MCP\/WebMCP-инструменты автоматизации\]\([^\n]+projects\/agent-portal\/\?lang=ru\)/);
  assert.match(llmsFullTxt, /#### F360 — основатель \/ руководитель студии · 2021–2022/);
  assert.match(llmsFullTxt, /#### MEGAVISOR \/ Gate9 — технический директор и сооснователь · октябрь 2011 — май 2014/);
  assert.doesNotMatch(llmsFullTxt, /ZIQ Design/);
  ...
  const agentPortalPublication = PUBLICATIONS.find(pub => pub.slug === 'agent-portal-retrospective');
  for (const locale of ['en', 'ru', 'es']) {
    assert.match(
      llmsFullTxt,
      new RegExp(`#### ${escapeRegExp(agentPortalPublication.locales[locale].title)}\\n\\nURL: .*\\/pulse\\/agent-portal-retrospective\\/\\?lang=${locale}\\n\\nPeriod: 2025-2026`),
    );
  }
