...
import { buildResourceTreeFromEntries } from 'symbiote-ui/contracts/resource-tree';
import {
...
test('synthetic project folders open their main article while keeping folder tree focus', () => {
  let groupTargets = [
    ['Projects/AI Tools', 'skills/agentic-ai'],
    ['Projects/Product Platforms', 'skills/product-ui'],
    ['Projects/Hardware', 'skills/hardware-capture'],
    ['Projects/Archived projects', 'skills/rnd-engineering'],
  ];
  let entries = new Map([
    ['projects/index', {}],
    ...groupTargets.map(([, targetId]) => [targetId, {}]),
  ]);
  let directorySelections = new Map([
    ['Projects', 'projects/index'],
    ...groupTargets,
  ]);

  for (let [folderId, targetId] of groupTargets) {
    let folderItem = { id: folderId, path: folderId };
    assert.equal(
      resolvePortfolioTreeSelection(folderItem, entries, directorySelections),
      targetId,
    );
    assert.equal(
      resolvePortfolioTreeHighlightId(targetId, [], directorySelections, folderItem.id),
      folderId,
    );
  }
  assert.equal(
    resolvePortfolioTreeSelection(
      { id: 'skills/agentic-ai', path: 'Skills/Loop engineering.md' },
      entries,
      directorySelections,
    ),
    'skills/agentic-ai',
...
test('tree selection scroll centers the row within available list space', () => {
  assert.equal(
    getCenteredTreeSelectionScrollTop({
      scrollTop: 80,
      clientHeight: 400,
...
test('tree clicks preserve list scroll while external selections request centering', () => {
  assert.equal(shouldCenterPortfolioTreeSelection('Projects/AI Tools'), false);
  assert.equal(shouldCenterPortfolioTreeSelection('skills/agentic-ai'), false);
  assert.equal(shouldCenterPortfolioTreeSelection(''), true);
  assert.equal(shouldCenterPortfolioTreeSelection(), true);
});
...
test('skill entries keep their own tree selection ahead of related project groups', () => {
  let directorySelections = new Map([
    ['Projects/AI Tooling', 'skills/agentic-ai'],
    ['Skills', 'skills/index'],
  ]);

  assert.equal(
    resolvePortfolioTreeHighlightId('skills/agentic-ai', [], directorySelections),
    'skills/agentic-ai'
  );
  assert.equal(
    resolvePortfolioTreeHighlightId('skills/index', [], directorySelections),
    'skills/index'
  );
});
...
test('publication kinds resolve to the exact six locale keys', () => {
  assert.deepEqual(
    ['retrospective', 'update', 'release', 'research-note', 'field-note', 'article']
