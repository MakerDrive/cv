...
test('semantic article block ids stay aligned across English, Russian, and Spanish', () => {
  ...
  for (let slug of Object.keys(PORTFOLIO_ARTICLE_MEDIA_PLACEMENTS)) {
    let ids = parsePortfolioArticleBlocks(loadProjectContent(slug, 'en')).map((block) => block.id);
    assert.ok(ids.length > 0, `${slug}:en`);
    for (let locale of ['ru', 'es']) {
      assert.deepEqual(
        parsePortfolioArticleBlocks(loadProjectContent(slug, locale)).map((block) => block.id),
        ids,
        `${slug}:${locale}`,
      );
    }
...
test('portfolio routes selected media into article content instead of a graph player', async () => {
  ...
  assert.match(source, /this\.viewer\.toggleAttribute\?\.\('data-loading', loading\)/);
  assert.match(source, /portfolioRuntime\.scheduleCanvasSync\(\{ focus, focusScope \}\)/);
  ...
  assert.match(source, /this\.articleMediaPendingFocusId = this\.activeArticleMediaId;/);
  ...
  assert.match(
    source,
    /removeEventListener\?\.\('scrollend', this\.articleMediaScrollEndListener\)/,
  );
  assert.match(source, /let supportsScrollEnd = 'onscrollend' in scrollRoot;/);
  assert.match(source, /this\.articleMediaFocusScheduler\?\.cancel\?\.\(\)/);
  assert.match(
    source,
  ...
  assert.match(
    source,
    /if \(this\.articleMediaIsScrolling\) \{\s*this\.articleMediaPendingFocusId = change\.mediaId;\s*return;/,
  );
  assert.match(
    source,
    /if \(!this\.articleMediaInitialTargetResolved\) \{\s*this\.articleMediaInitialTargetResolved = true;\s*if \(!this\.articleMediaIsScrolling\) return;/,
  );
  assert.match(observerRuntime, /this\.activateArticleMediaNode\(change\.mediaId, \{ fit: true \}\)/);
  assert.doesNotMatch(observerRuntime, /this\.articleMediaFocusScheduler\?\.schedule\?\.\(change\.mediaId\)/);
  ...
  assert.match(
    source,
    /return !portfolioRuntime\.articleMediaIsScrolling\s*&& mediaId === portfolioRuntime\.activeArticleMediaId;/,
  );
  ...
  assert.match(css, /\.portfolio-viewer\[data-loading\] code-block::after/);
