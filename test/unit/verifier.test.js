...
test('verifier enforces the main bundle budget and runtime Markdown asset boundary', () => {
  ...
  assert.equal(measured.raw, 12);
  assert.ok(measured.gzip > 0 && measured.gzip <= 100);
  assert.throws(
    () => verifyMainBundleSize('too large', { raw: 1, gzip: 100 }),
    /bytes raw; limit is 1/,
  );
  assert.throws(
    () => verifyMainBundleSize('too compressed', { raw: 100, gzip: 1 }),
    /bytes gzip; limit is 1/,
  );
  ...
  assert.throws(
    () => verifyRuntimeMarkdownAssetSeparation(
      'const asset = "js/markdown-viewer/index.js"; const markedHighlight = true;',
      'const markedHighlight = true; const message = "Markdown source is missing.";',
    ),
    /contains the Markdown renderer implementation/,
  );
});
...
test('full verifier success and failure scenarios', async () => {
  ...
  try {
    ...
    const writeSuccessFiles = async () => {
      ...
      await fs.mkdir(path.join(tmpDir, 'js/live-observation'), { recursive: true });
      await fs.writeFile(
        path.join(tmpDir, 'js/live-observation/index.js'),
        'const live = "js/live-observation/index.js";',
      );
