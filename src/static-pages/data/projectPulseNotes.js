export const PROJECT_PULSE_NOTES = Object.freeze({
  en: {
    ...
    'browser-x-mcp': {
      ...
      details: '## Publication context\n\n- Public anchor: Browser X belongs to the 2025 public MCP tooling line.\n- R&D point: agents need reliable page inspection, form interaction, screenshots, and structured browser evidence to verify UI work from live page state.\n- Transition: the browser layer feeds testing, portfolio verification, and Agent Portal workflows.',
    },
    'context-x-mcp': {
    ...
    'symbiote-node': {
      summary: 'Journal note on Symbiote Node as archived context for earlier RND-PRO Symbiote integration and package-organization work.',
      details: '## Publication context\n\n- Public anchor: Symbiote.js itself is external/reference context; this note is about an earlier RND-PRO package workspace, not authorship of the original Symbiote.js project.\n- R&D point: the archived workspace collected integration glue, migration tools, package organization experiments, and early links between UI primitives and engine prototypes.\n- Transition: the current active Symbiote connection is Workspace, UI, and Engine; Node is kept as historical context.',
    },
    'symbiote-engine': {
  ...
  ru: {
    'agent-portal': {
      summary: 'R&D-заметка о линии Agent Portal и loop engineering: от экспериментов с агентами к управляемому контуру автономной разработки.',
      details: '## Контекст публикаций\n\n- Публичные якоря: последние работы по Agent Portal фиксируют вход задачи через карточку, изолированные worktree, делегированное выполнение, completion proof, audit/rework циклы, release gates, cleanup, retry/backoff и выход к человеку.\n- R&D-смысл: работа формирует инженерный контур, где агентное выполнение остаётся наблюдаемым, обратимым и достаточно безопасным для реальных задач разработки.\n- Переход: эта линия собирает MCP-инструменты, граф проекта, agent pool, браузерную автоматизацию и team memory в одну продуктовую поверхность.',
    },
    'symbiote-video-studio': {
      ...
      details: '## Контекст публикаций\n\n- Публичный якорь: эта ветка представлена в портфолио как R&D-поверхность для продуктовых медиа.\n- R&D-смысл: видео быстро показывает те же ограничения, что и другие инструменты: хаотичные исходники, состояние правок, циклы ревью и необходимость интерфейса, который удерживает процесс видимым.\n- Переход: эксперименты питают более широкую линию продуктовых интерфейсов и ИИ-помощи в медиа-системах.',
    },
    'autobox-v1': {
      summary: 'R&D-заметка об AUTOBOX v1: оборудование, свет, протокол съемки и production-процесс для оцифровки культурного наследия.',
      details: '## Контекст публикаций\n\n- Публичные якоря: статьи RND-PRO про AUTOBOX v1 и нэцкэ были опубликованы позже самой работы; материалы OBJET.art дают публичную преемственность cultural-heritage направления.\n- R&D-смысл: ключевая задача состояла в превращении фотограмметрии в повторяемый музейный процесс: многоракурсная съемка, кастомный свет, точный цвет, обращение с объектами, выездные сканирования и передача в 3D-postproduction.\n- Переход: PhotoPizza и ComplexScan дали базу управления движением и автоматизации съемки; AUTOBOX перенес эту линию в музейную валидацию более высокого качества.',
    },
    complexscan: {
      summary: 'R&D-заметка о ComplexScan как коммерческом hardware-шаге между open-source PhotoPizza и музейными системами сканирования.',
      details: '## Контекст публикаций\n\n- Публичные якоря: видео 2019 года показывают стеклянный поворотный стол, web-управление, синхронизацию камеры, лазерное центрирование и проверки стабильности оси; поздняя статья RND-PRO собирает продуктовую историю.\n- R&D-смысл: проект перешёл от DIY/open hardware к производимой системе с прозрачным диском для 3D-сканирования, 360-съемки и предметной фотографии.\n- Переход: продуктовая линия дошла до первых поставок и наладки производства, а затем стала техническим мостом к системам уровня AUTOBOX.',
    },
    boothbot: {
      ...
      details: '## Контекст публикаций\n\n- Публичный якорь: BoothBot представлен проектной страницей; независимые даты из соцсетей стоит подтверждать отдельно перед точной хронологией.\n- R&D-смысл: проект автоматизировал практический процесс у клиента: бутылки на складе, управляемый свет, движение камеры, простая эксплуатация и автоматическая выдача каталожных изображений.\n- Переход: кейс показывает, что метод аппаратной автоматизации применим к 3D-сканированию, 360-медиа и обычным производственным процессам.',
    },
    photopizza: {
      summary: 'R&D-заметка о PhotoPizza как open-source hardware-линии, родившейся внутри MEGAVISOR и ставшей платформой для съемки и сканирования.',
      details: '## Контекст публикаций\n\n- Публичные якоря: ранние видео 2013 года, DIY-репозиторий 2015 года, JavaScript/control репозиторий 2017 года, YouTube-канал PhotoPizza и полная история diy.photopizza показывают эволюцию проекта.\n- R&D-смысл: проект проверял доступное повторяемое движение, синхронизацию камеры, контроллеры и прошивки, browser-control, документацию и практические фотограмметрические процессы.\n- Переход: PhotoPizza стала базой для ComplexScan, экспериментов AUTOBOX и более поздней поддержки пользователей, включая Android-путь для случая с потерянным IR-пультом.',
    },
    megavisor: {
      summary: 'R&D-заметка о MEGAVISOR как стартовом продуктовом контексте, где съемочная R&D-линия отделилась в железо и софт.',
      details: '## Контекст публикаций\n\n- Публичные якоря: LinkedIn-опыт, старые Facebook/YouTube-материалы и проектная страница RND-PRO помещают MEGAVISOR до последующей линии PhotoPizza и ComplexScan.\n- R&D-смысл: продукту требовался надежный production-процесс 360-медиа на складах клиентов, поэтому работа включала технологию съемки, свет, автоматизацию, фотографов и организацию выездного процесса.\n- Переход: PhotoPizza была придумана внутри этого ограничения и затем продолжилась как самостоятельный open-source hardware/software проект.',
    },
    'mcp-agent-portal': {
      summary: 'R&D-заметка об открытом MCP control plane, который предшествует и поддерживает продуктовую линию Agent Portal.',
      details: '## Контекст публикаций\n\n- Публичный якорь: репозиторий и npm-пакет показывают control-plane слой отдельно от более крупного продукта Agent Portal.\n- R&D-смысл: эксперимент отвечает на вопрос, какой контекст, инструменты и браузерные операции нужны агенту, прежде чем он сможет надежно работать внутри проекта.\n- Переход: выводы переходят в продуктовый портал, где MCP-слой становится частью общей среды оркестрации.',
    },
    'project-graph-mcp': {
      summary: 'R&D-заметка о графах проекта как компактном контексте для агентов, работающих в реальных кодовых базах.',
      details: '## Контекст публикаций\n\n- Публичный якорь: репозиторий/npm-линия делает слой code intelligence видимым как отдельный MCP-инструмент.\n- R&D-смысл: сырые файлы слишком шумные для агентной работы, поэтому проект проверяет графовые summary, структуру зависимостей, skeleton кода и evidence как более полезный контекст.\n- Переход: это стало одним из источников контекста внутри Agent Portal и loop-engineering процесса.',
    },
    'agent-pool-mcp': {
      summary: 'R&D-заметка о делегировании CLI-работы нескольким агентам при сохранении владения, состояния и проверяемости.',
      details: '## Контекст публикаций\n\n- Публичный якорь: пакет Agent Pool документирует исполняющий слой для многоагентной работы.\n- R&D-смысл: делегирование полезно только тогда, когда владение задачей, состояние процесса, результаты и сбои достаточно видимы для следующего решения оркестратора.\n- Переход: эта линия поддерживает маршрутизацию задач Agent Portal и контролируемые параллельные циклы разработки.',
    },
    'browser-x-mcp': {
      summary: 'R&D-заметка о браузерной автоматизации как сборе evidence для агентов.',
      details: '## Контекст публикаций\n\n- Публичный якорь: Browser X относится к публичной MCP-линии инструментов 2025 года.\n- R&D-смысл: агентам нужна надежная инспекция страниц, работа с формами, скриншоты и структурированные доказательства UI для проверки интерфейса по состоянию живой страницы.\n- Переход: браузерный слой используется в тестировании, проверке портфолио и рабочих процессах Agent Portal.',
    },
    'context-x-mcp': {
      summary: 'R&D-заметка о выборе контекста перед выполнением: подобрать правильную память, файлы и поверхность инструментов.',
      details: '## Контекст публикаций\n\n- Публичный якорь: Context X - один из публичных MCP-инструментов ранней агентной линии.\n- R&D-смысл: многие сбои агентов начинаются до выполнения, когда форма задачи, проектная память и релевантные файлы выбраны плохо.\n- Переход: эта идея стала частью более широкой team-memory и orchestrated-agent практики.',
    },
    'terminal-x-mcp': {
      summary: 'R&D-заметка о терминальной автоматизации для планового выполнения команд, валидации и безопасной обратной связи агентам.',
      details: '## Контекст публикаций\n\n- Публичный якорь: Terminal X входит в публичный MCP-набор для агентных execution-сред.\n- R&D-смысл: выполнение команд требует планирования, мониторинга, снятия ошибок, структурированного отчёта и видимой границы контроля вокруг проверочных запусков.\n- Переход: тот же паттерн позже появляется в Agent Portal gates, audits и cleanup flows.',
    },
    'symbiote-workspace': {
      summary: 'R&D-заметка о Symbiote Workspace как активной продуктовой поверхности, связывающей Symbiote UI и Symbiote Engine.',
      details: '## Контекст публикаций\n\n- Публичный якорь: ветка Symbiote Workspace - текущая RND-PRO линия, связанная с интерфейсами, которые могут собирать агенты.\n- R&D-смысл: эксперимент про продуктовые поверхности, которые агенты могут собирать и понимать: долговременное состояние, переиспользуемая структура графа и переносимые артефакты.\n- Переход: эта ветка связывает Symbiote UI, состояние графа, Symbiote Engine и portal-работу в модель рабочего пространства; Symbiote Node остается архивным контекстом.',
    },
    'symbiote-ui': {
      summary: 'R&D-заметка о Web Components и provider metadata как UI-контракте для продуктов, собираемых агентами.',
      details: '## Контекст публикаций\n\n- Публичный якорь: пакет/репозиторий Symbiote UI показывает переиспользуемый UI-слой, который используется в этом CV и связанных RND-PRO интерфейсах.\n- R&D-смысл: агентам нужны UI-примитивы с discoverable contracts, стабильными metadata, предсказуемым поведением и правилами переиспользуемой композиции.\n- Переход: компонентный слой поддерживает Agent Portal, поверхности CV-портфолио, графовые инструменты, деревья, панельные раскладки и будущие WebMCP-facing UI.',
    },
    'symbiote-node': {
      summary: 'R&D-заметка о Symbiote Node как архивном контексте ранней интеграции и package-organization в линии RND-PRO Symbiote.',
      details: '## Контекст публикаций\n\n- Публичный якорь: Symbiote.js является внешним/reference-контекстом; эта заметка относится к более раннему RND-PRO package workspace, а не к авторству исходного проекта Symbiote.js.\n- R&D-смысл: архивный workspace собирал интеграционную обвязку, миграционные инструменты, эксперименты с организацией пакетов и первые связи между UI-примитивами и прототипами engine.\n- Переход: актуальная Symbiote-связка - Workspace, UI и Engine; Node сохраняется как исторический контекст.',
    },
    'symbiote-engine': {
      summary: 'R&D-заметка о Symbiote Engine как исполняемом слое в актуальной связке Symbiote Workspace / UI / Engine.',
      details: '## Контекст публикаций\n\n- Публичный якорь: ветка Symbiote Engine представлена как текущая RND-PRO package/repository линия.\n- R&D-смысл: вопрос в том, насколько далеко визуальный граф может пройти от схемы до реальной исполняемой модели повторяемого процесса.\n- Переход: engine связывает графовое представление, состояние Workspace и продуктовые UI-инструменты автоматизации.',
    },
    'photopizza-remote': {
      summary: 'R&D-заметка о переносе управления PhotoPizza в браузер, чтобы эксплуатация в поле была практичной вне лаборатории.',
      details: '## Контекст публикаций\n\n- Публичный якорь: репозиторий PhotoPizza Remote отдельно показывает browser-control сторону hardware-экосистемы.\n- R&D-смысл: поворотный стол полезен в production только тогда, когда оператор может задавать параметры съемки, управлять движением и восстанавливать процесс без работы на уровне прошивки.\n- Переход: этот UI/control паттерн позже повторяется в интерфейсах аппаратной автоматизации и agent-operable tools.',
    },
    'photosnail-public': {
      ...
      details: '## Контекст публикаций\n\n- Публичный якорь: GitHub-репозиторий 2016 года сохраняет ранний web/project артефакт линии PhotoSnail.\n- R&D-смысл: это раннее исследование управляемого движения камеры и медиа-презентации вокруг автоматизированной съемки.\n- Переход: проект относится к той же до-RND-PRO дуге медиа-автоматизации, что F360, MEGAVISOR и PhotoPizza.',
    },
    'lifecycle-messaging-platform': {
      ...
      details: '## Контекст публикаций\n\n- Публичный якорь: эта заметка является публичным якорем проекта; клиентские/source-ссылки намеренно не указаны.\n- R&D-смысл: работа представлена через класс задач: согласованные клиентские коммуникации, lifecycle-сообщения, opt-in SMS-сценарии, оркестрация кампаний, сегментация, аналитика, роли и автоматизация процессов.\n- Переход: этот кейс расширяет портфолио от медиа/hardware и ИИ-инструментов к конфиденциальным продуктовым платформам с безопасной публичной границей.',
    },
  },
  es: {
    'agent-portal': {
    ...
    'browser-x-mcp': {
      ...
      details: '## Contexto de publicación\n\n- Ancla pública: Browser X pertenece a la línea pública de herramientas MCP de 2025.\n- Punto I+D: los agentes necesitan inspección fiable de páginas, formularios, screenshots y evidencia estructurada de UI para verificar interfaces desde el estado real de la página.\n- Transición: la capa de navegador alimenta pruebas, verificación del portafolio y flujos de Agent Portal.',
    },
    'context-x-mcp': {
