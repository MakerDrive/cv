...
[data-retirement-locale] {
  display: none;
  width: min(100% - 32px, 720px);
  margin: 0 auto;
  padding-block: clamp(48px, 10vh, 112px);
}

:root:lang(en) [data-retirement-locale="en"],
:root:lang(ru) [data-retirement-locale="ru"],
:root:lang(es) [data-retirement-locale="es"] {
  display: block;
}
...
.portfolio-viewer[data-loading] code-block {
  position: relative;
}
...
.portfolio-viewer[data-loading] code-block::after {
  content: '';
  position: absolute;
  inset: 24px 28px;
  border-radius: 6px;
  pointer-events: none;
  background:
    linear-gradient(var(--pulse-outline) 0 0) 0 0 / 38% 26px no-repeat,
    linear-gradient(var(--pulse-outline) 0 0) 0 60px / 100% 14px no-repeat,
    linear-gradient(var(--pulse-outline) 0 0) 0 88px / 86% 14px no-repeat,
    linear-gradient(var(--pulse-outline) 0 0) 0 116px / 93% 14px no-repeat,
    linear-gradient(var(--pulse-outline) 0 0) 0 170px / 100% 14px no-repeat,
    linear-gradient(var(--pulse-outline) 0 0) 0 198px / 78% 14px no-repeat;
}
...
[tour-highlight] {
  position: absolute;
  pointer-events: none;
  border: 3px solid var(--sn-sys-accent, var(--pulse-accent));
  border-radius: var(--sn-radius-sm, 6px);
  transition: all var(--sn-transition-normal) var(--sn-transition-easing);
  z-index: 9999;
  box-sizing: border-box;
  background: color-mix(in oklch, var(--sn-sys-accent, var(--pulse-accent)) 10%, transparent);
}
`;
