const PULSE_KIND_MESSAGE_KEYS = Object.freeze({
  ...
  article: 'portfolio.pulse.type.article',
});
 ...
 * @param {string} [treeSelectionId]
 */
export function resolvePortfolioTreeHighlightId(
  selectedId,
  publications,
  directorySelections = [],
  treeSelectionId = '',
) {
  let selectedTreeId = String(treeSelectionId || '').trim();
  if (selectedTreeId) return selectedTreeId;
  ...
  if (id.startsWith('skills/')) return id;
 ...
 * @returns {string}
 */
export function resolvePortfolioTreeSelection(item, entries, directorySelections = []) {
  if (!item) return '';
  if (item.metadata?.targetId) return item.metadata.targetId;
  if (entries?.has(item.id)) return item.id;
  if (entries?.has(item.path)) return item.path;
  let selections = directorySelections instanceof Map
    ? directorySelections
    : new Map(directorySelections);
  return selections.get(item.id) || selections.get(item.path) || '';
}

export function shouldCenterPortfolioTreeSelection(treeSelectionId = '') {
  return !String(treeSelectionId || '').trim();
}
...
export function getCenteredTreeSelectionScrollTop({
  scrollTop = 0,
  clientHeight = 0,
  scrollHeight = 0,
  rowOffset = 0,
  ...
  let maxScrollTop = Math.max(0, Number(scrollHeight) - Number(clientHeight));
  let centeredScrollTop = Number(scrollTop) + Number(rowOffset)
    - ((Number(clientHeight) - Number(rowHeight)) / 2);
